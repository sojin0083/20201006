...
<title>S-LINDA 결과지</title>
<c:import url="/cmmn/importResourceFile.do" var="jsimport"
	charEncoding="utf-8">
	<c:param name="pageMode">chart</c:param>
</c:import>
...
<script type="text/javascript">
	...
	function setTrgterSExamInfo(data) {
		...
		//$("#examOpin").html(data.EXAM_OPIN);
		$('#examOpin').html(data.EXAM_OPIN.split('&lt;br/&gt;').join("<br/>"));
	...
	function updateExamOpin() {
		...
		cfn.ajax({
			...
			"data" : {
				"EXAM_NO" : EXAM_NO,
				"EXAM_OPIN" : $("#examOpin").val()

			},
			"method" : "POST",
			"dataType" : "JSON",
			"success" : function(data) {
				alert("평가 종합 소견이 등록되었습니다.");
			},
			"error" : function(data) {
				alert("통신실패");
			}
	...
	function setTrgterSExamGrpO(data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			str += "<li>";
			str += "<div class='title_value'><span>" + data[i].AREA_NM
					+ "</span><span>" + data[i].VAL + "</span></div>";
			...
			if (data[i].Z_POINT_DIR == "R") {
				...
				str += "</li>";
				str += "<li>";
				str += "<div class='le_g grp_blue' style='width:" + bar
						+ "%'></div>";
				str += "</li>";
			} else if (data[i].Z_POINT_DIR == "L") {
				str += "<li>";
				str += "<div class='ri_g grp_blue' style='width:" + bar
						+ "%'></div>";
				str += "</li>";
				str += "<li>";
	...
	function setTrgterSExamGrpM(data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			str += "<li>";
			str += "<div class='title_value'><span>" + data[i].EXAM_KEY_WORD
					+ "</span><span>" + data[i].VAL + "</span></div>";
			str += "<div class='stack_graph'>";
			str += "<ul>";
			if (data[i].Z_POINT_DIR == "R") {
	...
	function setTrgterSExamArea(data) {
		...
		str2 += "<td>"
				+ (Math.round(AREA_Z_POINT * 1e12) / 1e12 / data.length)
						.toFixed(2) + "</td>"
		str2 += "</tr>"
	...
	function setTrgterSExamReportDtls(data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			str += " class='" + data[i].AREA_NM + "_NM'>" + data[i].AREA_NM
					+ "</th>"
			str += "<th scope='colgroup' class='line " + data[i].AREA_DTLS_NM + "_DTLS_NM'>"
					+ data[i].AREA_DTLS_NM + "</th>"
	...
	function setTrgterSExamReportRec(data) {
		...
		for (var i = 0; i < data.length; i++) {
			str += "<tr>"
			str += "<th scope='row' style='text-align:center' class='" + data[i].AREA_NM + "_Name'>"
					+ data[i].AREA_NM + "</th>"
	...
	function beforePrint() {
		...
		document.body.innerHTML = wrap_content.innerHTML;
	}
	...
	<div class="subpage">
		<section>
			<ul class="title_align">
				...
				<li><a href="#" class="btn_all blue btn_print"
					onclick="printArea();">인쇄</a></li>
			</ul>
...
<div class="page">
	<div class="subpage">
		<h2>세부 검사 결과표</h2>
		<table class="table_type print topb">
			<colgroup>
				<col width="14%">
				<col width="13%">
				<col width="*">
